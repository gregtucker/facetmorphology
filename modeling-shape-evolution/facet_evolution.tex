...
%Ok, at this point we have a conundrum. It turns out that the actual dissolution rate on a flat surface asymptotes to around 3.6 delta s, for reasons that remain obscure to me but presumably relate to surface roughness. So, the earlier "fit" was apparently a coincidence. One option is to calculate the erosion rate using my erosion function, and compare with facet angle. But of course they should correlate, since it amounts to basically the same method (or variation on same). I could nonetheless do this and back out the effective factor. But why is the factor actually a bit less than 2? Did I make a mistake in the analysis?
...
We consider the case in which rock weathers iso-volumetrically to regolith, with regolith motion driven by stochastic disturbance events (Figure~\ref{fig:cellstates}). Two dimensionless parameters determine the model's behavior:
\begin{eqnarray*}
