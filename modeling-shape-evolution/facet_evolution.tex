...
Fault scarps and facets have intrigued geologists since at least the late 19th century, %when exploration of the western United States revealed the basin and range 
...
[NOTE WE DON'T CONSIDER INCISED FACETS, OR THOSE ON WHICH INCISION-LIKE PROCESSES AND CONCENTRATED FLOW PLAY A MAJOR ROLE]

[ALSO SAY STHING ABOUT WHAT WE MEAN BY FACET: BASICALLY, FAULT SCARP FACING BASINWARD AND UNMODIFIED BY INCISION]
...
Facet shape depends on both $w'$ and $d'$. With sufficiently effective weathering and transport, represented by the parameter space $w'>1$ and $d'>1$, the model produces somewhat convex-upward facets (Figure~\ref{fig:dwprofiles}, upper right). If either of these parameters is less than unity, the model generates planar facets.
...
can a model that combines rock weathering with disturbance-driven soil creep account for the observed range in facet angle, regolith cover fraction, shape, and colluvial wedge angle? If so, what are the primary controlling factors? 

yes: runs show variation in angle, reg cover, shape, and colluv angle. all are controlled by two dimensionless params.

Does the model imply a systematic relationship between facet angle and erosion rate, and if so what does that relationship look like? 

not yet, or at least not directly... need to plot data in a different way for that. in a sense one could simply plot angle vs $1/w'$, because that then is effectively a dimensionless slip rate. the problem is that the erosion rate IS the angle, or at least proportional, so where do $w'$ and $d'$ come in? i suppose one could use the fact that at steady state the vertical erosion rate is just $V \cos \alpha$. and define a dimensionless vertical erosion rate $(V/w) \cos \alpha$. then plot facet angle against this... [THINK ABOUT THIS MORE: IS THERE A BETTER WAY? problem of course is that the dimensional erosion rate is the same for all, it's just the dimensionless version that varies. Basically, we have 4 dimensional parameters, $V$ (or $\tau$), $d$, $w$, and $\delta$, so two groups, and our (dimensionless) outcome, $\theta$, is a function of those two.]
